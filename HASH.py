...
def fraseToSHA256(frase):
    return hashlib.sha256(frase.encode('utf-8')).hexdigest()

def fraseToMD5(frase):
    return hashlib.md5(frase.encode('utf-8')).hexdigest()

def verifica_dic(dict):
    for i in range(len(dict)):
        frase = dic[i]["frase"]
        sha256 = dic[i]["sha256"]
        md5 = dic[i]["md5"]

        sha256Obtido = fraseToSHA256(frase)
        MD5Obtido = fraseToMD5(frase)
        print("Frase {0}: {1}\n".format(i,frase))
        print("SHA256 Frase {0}: {1}\n".format(i, sha256))
        print("MD5 Frase {0}: {1}\n".format(i, md5))
        print("SHA256 OBTIDO: {0}\n".format(sha256Obtido))
        print("MD5 OBTIDO: {0}\n".format(MD5Obtido))
        if(fraseToSHA256(frase) == sha256 and fraseToMD5(frase) == md5):
